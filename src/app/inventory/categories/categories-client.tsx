...
import { Tag, Plus, Trash2, Layers, Search } from 'lucide-react';
...
export default function CategoriesClient({ initialCategories }: { initialCategories: any[] }) {
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCategories(prev => [...prev, { ...newCat, products: [] }].sort((a, b) => a.name.localeCompare(b.name)));
      setIsAddOpen(false);
      setName('');
    } catch (err: any) {
  ...
  const handleDeleteCategory = async (id: string, productCount: number) => {
    if (productCount > 0) {
      alert(`Cannot delete this category because it is currently assigned to ${productCount} product(s). Please reassign the products first.`);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-foreground">Product Categories</h1>
          <p className="text-muted-foreground text-sm">Classify and organize your products into inventory groups.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map(c => {
          ...
          return (
            <div key={c.id} className="p-6 bg-card text-card-foreground rounded-3xl border border-border shadow-sm flex flex-col justify-between space-y-4 group hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start">
                ...
                <button
                  onClick={() => handleDeleteCategory(c.id, productCount)}
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="max-w-md rounded-3xl p-6">
          ...
          <form onSubmit={handleAddCategory} className="space-y-4 pt-2">
            <div>
              <label className="text-xs font-bold text-muted-foreground uppercase mb-1 block">Category Name *</label>
              <Input
                placeholder="e.g. Aluzinc Sheets, Anti-Rust Sheets"
                value={name}
