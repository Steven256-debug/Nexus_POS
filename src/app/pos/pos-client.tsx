...
export default function PosClient({ initialProducts }: { initialProducts: any[] }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-full min-h-[600px] relative">
      <div className="flex-1 flex flex-col bg-white rounded-3xl shadow-sm border border-zinc-200 overflow-hidden relative pb-20 lg:pb-0">
        <div className="p-5 border-b border-zinc-100 flex flex-col gap-4 bg-zinc-50/80 backdrop-blur-md z-10">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-400" />
            <Input 
              placeholder="Search products by name or SKU..." 
              className="pl-12 h-14 text-lg rounded-2xl bg-white shadow-sm border-zinc-200 focus-visible:ring-blue-500"
              value={searchTerm}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 bg-zinc-50/30">
                ...
                <div 
                  ...
                  className={`relative flex flex-col p-5 rounded-2xl border transition-all duration-300 ${
                    isOutOfStock 
                      ...
                      : 'border-zinc-200 bg-white hover:border-blue-400 hover:shadow-xl hover:shadow-blue-500/10 cursor-pointer active:scale-[0.98]'
                  }`}
                >
                  <div className="flex justify-between items-start mb-4">
                    <span className="text-xs font-mono font-medium text-zinc-500 bg-zinc-100 px-2.5 py-1 rounded-lg">{product.sku}</span>
                    <span className="font-bold text-xl text-blue-600">${product.pricePerUnit.toFixed(2)}</span>
                  </div>
                  {product.imageUrl && (
                    <div className="mb-4 -mx-1 -mt-2 rounded-xl overflow-hidden h-32 bg-zinc-100 flex items-center justify-center">
                      <img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover" />
                    </div>
                  )}
                  <h3 className="font-semibold text-zinc-900 leading-tight mb-4 flex-1 text-lg">{product.name}</h3>
      ...
      {cart.length > 0 && (
        <div className="lg:hidden fixed bottom-20 left-4 right-4 z-40 pb-safe">
          ...
          >
            ...
            <span className="font-bold bg-white/20 px-3 py-1.5 rounded-lg border border-white/10 shadow-inner">
              ${total.toFixed(2)}
            </span>
          </Button>
        </div>
      ...
      `}>
        ...
        <div className={`
          bg-white flex flex-col shadow-2xl relative z-10 transition-transform duration-300 transform
          ${isMobileCartOpen ? 'h-[85vh] rounded-t-3xl translate-y-0' : 'rounded-3xl h-full border border-zinc-200'}
        `}>
          <div className={`p-6 border-b border-zinc-100 bg-zinc-950 text-white flex justify-between items-center ${isMobileCartOpen ? 'rounded-t-3xl' : ''}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-zinc-50/50">
          {cart.length === 0 ? (
            ...
            <div className="space-y-3">
              {cart.map(item => (
                <div key={item.id} className="flex flex-col p-4 bg-white rounded-2xl border border-zinc-100 shadow-sm gap-3 group">
                  <div className="flex justify-between items-start gap-4">
                    <h4 className="font-semibold text-zinc-900 leading-tight">{item.name}</h4>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-1 bg-zinc-50 border border-zinc-200 rounded-xl p-1">
                      <button onClick={() => updateQuantity(item.id, -1)} className="p-2 hover:bg-white rounded-lg transition-colors shadow-sm" disabled={item.quantity <= 1}>
                      ...
                      <input 
                        ...
                        className="w-12 text-center font-bold text-zinc-900 bg-transparent border-none focus:outline-none focus:ring-1 focus:ring-blue-500 rounded-md [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                      />
                      <button onClick={() => updateQuantity(item.id, 1)} className="p-2 hover:bg-white rounded-lg transition-colors shadow-sm" disabled={item.quantity >= item.maxStock}>
      ...
      <Dialog open={isCheckoutOpen} onOpenChange={setIsCheckoutOpen}>
        <DialogContent className="max-w-md rounded-3xl p-8">
          ...
          <div className="py-6">
            <div className="text-center mb-10 p-6 bg-zinc-50 rounded-2xl border border-zinc-100">
              <p className="text-zinc-500 text-sm font-semibold uppercase tracking-wider mb-2">Amount Due</p>
            ...
            <div className="grid grid-cols-3 gap-4">
              <button 
                ...
                className={`flex flex-col items-center justify-center p-5 rounded-2xl border-2 transition-all duration-200 ${paymentMethod === 'CASH' ? 'border-blue-600 bg-blue-50 text-blue-700 shadow-md scale-105' : 'border-zinc-200 hover:border-blue-300 text-zinc-600 hover:bg-zinc-50'}`}
              >
              ...
              <button 
                ...
                className={`flex flex-col items-center justify-center p-5 rounded-2xl border-2 transition-all duration-200 ${paymentMethod === 'CARD' ? 'border-blue-600 bg-blue-50 text-blue-700 shadow-md scale-105' : 'border-zinc-200 hover:border-blue-300 text-zinc-600 hover:bg-zinc-50'}`}
              >
              ...
              <button 
                ...
                className={`flex flex-col items-center justify-center p-5 rounded-2xl border-2 transition-all duration-200 ${paymentMethod === 'MOBILE_MONEY' ? 'border-blue-600 bg-blue-50 text-blue-700 shadow-md scale-105' : 'border-zinc-200 hover:border-blue-300 text-zinc-600 hover:bg-zinc-50'}`}
              >
          ...
          <DialogFooter className="gap-3 sm:gap-0 mt-4">
            <Button variant="ghost" className="h-14 w-full rounded-xl text-zinc-500 font-semibold" onClick={() => setIsCheckoutOpen(false)}>Cancel</Button>
