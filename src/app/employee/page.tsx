...
export default async function EmployeeDashboard() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-4xl font-bold tracking-tight text-zinc-900 mb-2">Employee Dashboard</h1>
        <p className="text-zinc-500 text-lg">Welcome back, {user.email.split('@')[0]}. Here is your performance for today.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 bg-white rounded-2xl border border-zinc-200 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-sm font-medium text-zinc-500">Your Revenue Today</p>
              <h3 className="text-2xl font-bold text-zinc-900">${stats.todayRevenue.toFixed(2)}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 bg-white rounded-2xl border border-zinc-200 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-sm font-medium text-zinc-500">Your Sales Today</p>
              <h3 className="text-2xl font-bold text-zinc-900">{stats.todaySalesCount}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 bg-white rounded-2xl border border-zinc-200 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-sm font-medium text-zinc-500">Items Sold Today</p>
              <h3 className="text-2xl font-bold text-zinc-900">{stats.totalItemsSold}</h3>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-3xl border border-zinc-200 shadow-sm p-6">
            ...
            <div className="space-y-4">
              ...
              <button disabled className="w-full group flex items-center justify-between p-4 bg-zinc-50 text-zinc-500 border border-zinc-200 rounded-2xl cursor-not-allowed">
                <div className="flex items-center gap-3">
            ...
            <div className="space-y-4">
              {recentSales.map(sale => (
                <Link href={`/receipt/${sale.id}`} key={sale.id} className="block group">
                  <div className="flex justify-between items-center p-4 bg-zinc-50 hover:bg-blue-50/50 rounded-xl border border-zinc-100 transition-colors">
                    <div>
                      <p className="font-medium text-zinc-900 group-hover:text-blue-700 transition-colors">Receipt #{sale.id.slice(0, 8).toUpperCase()}</p>
                    ...
                    <div className="flex items-center gap-4">
                      <p className="font-bold text-zinc-900 group-hover:text-blue-700">${sale.totalAmount.toFixed(2)}</p>
